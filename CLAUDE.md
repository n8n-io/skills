...
- `opencode/plugin.ts` + `opencode/README.md` + root `package.json`: the OpenCode plugin. TypeScript glue that bridges the canonical `hooks/` bash scripts into OpenCode's plugin events (`config`, `experimental.chat.system.transform`, `experimental.session.compacting`, `tool.execute.after`). The root `package.json` (`main: opencode/plugin.ts`) makes the repo installable as one line: `"plugin": ["n8n-skills@git+https://github.com/n8n-io/skills.git"]`; the `config` hook self-registers the bundled `skills/` dir so no `skills.paths` entry is needed. Derives its tool-matcher table from `hooks/hooks.json` at load, so a new hook there propagates without editing the plugin. Resolves the repo root via `dirname(fileURLToPath(import.meta.url))` (works under both Bun and the Node/Electron desktop app; Bun's `import.meta.dir` is undefined under Node and would disable the plugin). Two of the transform events are experimental OpenCode surface, prone to breaking across versions.
