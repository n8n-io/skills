...
Set the response code explicitly on every Respond branch. See `n8n-error-handling`'s `RESPONSE_SHAPES.md` for the success/4xx/5xx mapping.
...
For 202-Accepted-plus-callback and other async API patterns, see `n8n-error-handling`'s `API_WORKFLOWS.md`.
