...
- Putting tokens in `headerParameters` / `queryParameters` / `bodyParameters` instead of credentials: this puts them in plain text in the workflow. See `n8n-credentials-and-security`.
...
For the full auth pattern, see `n8n-credentials-and-security`'s `HTTP_REQUEST_WITH_AUTH.md`.
...
Built-in. See `n8n-loops` `HTTP_PAGINATION.md` for the full matrix. Don't reinvent with `Loop Over Items` + `$pageCount` unless the API does something the modes can't express.
