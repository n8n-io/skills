...
- If only 0 or 1 downstream consumer references each field, **delete the Set node and inline the expression at the consumer**. The most common antipattern (`n8n-expressions` non-negotiable #2).
...
- If it's pure data shaping (`.map`, `.filter`, `.find`, field renaming, optional chaining), rewrite as expression or Edit Fields with arrow function. Code's bar is multi-source aggregation, external libraries, or stateful transforms (`n8n-code-nodes`).
...
Invoke `n8n-error-handling` if any of these are true:
...
If you find any, invoke `n8n-credentials-and-security` and migrate before publishing.
