...
See `n8n-binary-and-data` for binary patterns. Code nodes are one of several ways to handle binary, often not the best.
...
Better: let Code throw, set `onError: 'continueErrorOutput'` and wire `output(1)`. See `n8n-error-handling`.
