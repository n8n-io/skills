...
1. **Wrap n8n capabilities the MCP doesn't expose.** The MCP covers workflow CRUD, validation, execution, data tables, credential listing, execution search, folder/project listing, and tag listing (`list_tags`) and tag-filtered workflow search. Still missing: folder CRUD, tag create/delete/get, instance metadata, credential creation. Build a workflow that hits the n8n API and exposes the result as an agent tool.
...
- Tag create / delete / get: REST API exists, no MCP tool yet. (Tag listing is now natively available via `list_tags`.)
...
1. **Discover** via `search_workflows({ query: '<keyword>' })`. Workflows with MCP access on return `availableInMCP: true`. Filter for that. You can also filter by tag using `search_workflows({ tags: ['<tag-name>'] })`, or enumerate all tags first with `list_tags`.
