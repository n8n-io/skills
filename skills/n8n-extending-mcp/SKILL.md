...
1. **Wrap n8n capabilities the MCP doesn't expose.** The MCP covers workflow CRUD, validation, execution, data tables, credential listing, execution search, folder/project listing, and tag listing. Still missing: folder CRUD, tag create/rename/delete, instance metadata, credential creation. Build a workflow that hits the n8n API and exposes the result as an agent tool.
...
- Tag create/rename/delete: REST API exists, no MCP tool yet. (Tag **listing** is now natively available via `list_tags`.)
...
1. **Discover** via `search_workflows({ query: '<keyword>' })`. Workflows with MCP access on return `availableInMCP: true`. Filter for that. You can also narrow by tag using `search_workflows({ tags: ['<tag-name>'] })` or discover available tags first with `list_tags`.
