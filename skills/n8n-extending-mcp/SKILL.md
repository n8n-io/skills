...
- Tag create/update/delete: REST API exists, no MCP tool yet. (Tag listing is now native via `list_tags`.)
...
### 2. Tag management (create / update / delete)

The MCP now natively exposes `list_tags` (read-only). Tag mutation (create, rename, delete) still requires a REST API wrap.
