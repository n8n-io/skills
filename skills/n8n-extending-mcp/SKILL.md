...
1. **Wrap n8n capabilities the MCP doesn't expose.** The MCP covers workflow CRUD, validation, execution, data tables, credential listing, execution search, folder/project listing, and tag listing. Still missing: folder CRUD, tag create/update/delete, instance metadata, credential creation. Build a workflow that hits the n8n API and exposes the result as an agent tool.
...
- Tag create/update/delete: REST API exists, no MCP tool yet. (Tag listing is now natively available via `list_tags`.)
...
- Things the MCP already exposes natively (including `list_tags` for tag listing).
...
### 2. Tag create / update / delete

The MCP natively exposes `list_tags` for reading tags. Creating, updating, or deleting tags still requires wrapping the n8n REST API.
...
| Building a wrapper for tag listing when `list_tags` is already a native MCP tool | Unnecessary duplication | Use the native `list_tags` tool directly |
