...
1. **Wrap n8n capabilities the MCP doesn't expose.** The MCP covers workflow CRUD, validation, execution, data tables, credential listing, execution search, folder/project listing, and tag listing (`list_tags`) and tag-filtered workflow search (`search_workflows` with `tags` parameter). Still missing: folder CRUD, tag CRUD (create/delete/get individual tag), instance metadata, credential creation. Build a workflow that hits the n8n API and exposes the result as an agent tool.
...
- Tag CRUD (create, delete, get individual tag): REST API exists, no MCP tool yet. Note: `list_tags` is now a native MCP tool — use it directly instead of wrapping.
...
| Building a tag-listing wrapper when `list_tags` is a native MCP tool | Unnecessary workflow, duplicates native capability | Use `list_tags` directly |
