...
description: "Use when you want to identify the problems, intents and interaction patterns in how you work with coding agents (Claude Code, Codex, Cursor, OpenCode, Pi, Gemini CLI and others): what frustrates you, what you ask for, how you steer and check status, which integrations you use, and where a UI would beat chat. Four open questions, the rest from your local history, counts only. Also use it on a folder of such reports from several people to assess the problems across them."
---
...
4. **Interview: four open questions, one message, free text, one probe at most.** The interview looks for problems, not solutions. It offers no options, no examples and no interface or use-case names, because those steer people toward our answers; the person's own words are the material that agents assess later across everyone. Post one plain chat message and stop; do not use AskUserQuestion or any option widget on any harness:
   > Four questions, a few minutes. Answer in your own words, as long or short as you like; skip any.
...
| 8 In my own words | The four open answers, verbatim |
...
| Opening the interview without saying how long it is | Four questions, one message, a few minutes; the message says so. |
| Asking about use-cases, options or features | The interview finds problems. Four open questions, no options, no suggestions; solutions are assessed later across people. |
| Probing every short answer, or probing twice | One probe, one question, only under about fifteen words, never on a skipped question. |
