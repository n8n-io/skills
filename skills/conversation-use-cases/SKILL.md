...
4. **Interview: four open questions, one message, free text.** The interview looks for problems, not solutions, so it offers no options, no examples and no interface or use-case names; the person's own words are the material that agents assess later across everyone. Post one plain chat message and stop; do not use AskUserQuestion or any option widget on any harness:
   > Four questions, about five minutes. Answer in your own words, as long or short as you like; skip any.
   ...
   Wait for the reply. Record it verbatim in section 8, one cell per question, empty when skipped. If the person asks what a question means, rephrase it without naming any feature, interface or use-case. Never summarise or tidy their words.
...
| Opening the interview without saying how long it is | Four questions, one message, about five minutes; the message says so. |
...
## Assessing several reports

When reports from several people are in, do not read them all in one context. Dispatch one subagent per report with the same brief: return that person's problems as one-line statements, quoting section 8 where it helps (the person wrote it for this), each with the section 2, 3, 4 and 6 numbers that support or contradict it. Then one subagent over all those returns: cluster the problems across people, count how many people share each cluster, name the strongest disagreement, and list which cards in `references/use-cases.md` each cluster points at and which clusters no card covers. The uncovered clusters are where new use-cases come from.
