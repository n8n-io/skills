...
3. **Read intents, not sentences.** `python3 scripts/session_miner.py --sample-turns 20 --since-days 60` streams, for every ask category, up to 20 redacted human turns drawn across all readable sessions in the window (most recent first, one turn per session per pass, sessions with a single turn skipped), plus up to 20 turns with frustration markers. Nothing is saved; secret-shaped strings are redacted. Read it once, whole. For every category in the report write one line describing the typical intent in your own words (for example "wants to know what is still missing after a long autonomous run", "hands a bulleted QA list with screenshots and expects each item addressed"); for the frustration sample write one line on what usually preceded it and check it against the Frustration signals table. Then read two or three whole sessions with `python3 scripts/session_miner.py --user-turns <session_id>` (the most recent with 10 or more `user_turns` in the JSON) for flow: how a status ask goes, how feedback arrives, how decisions get answered. Never copy a sentence, never name a person or a customer. If a category has fewer than five sampled turns, say the intent line rests on a thin sample.
4. **Interview.** Section 8 of `references/report-template.md`, with AskUserQuestion, at most four questions per call. On an agent without AskUserQuestion (Codex, Cursor, OpenCode and others), ask the same questions in chat, four at a time, each with lettered options and a free-text line, and wait for the answer before the next batch.
   - **Say what is coming first.** Work out which cards fire, then open with one message: the total number of questions, the batches (five core, five concerns, N cards, then follow-ups for cards that fit), the expected time, and that any question can be skipped or the rest stopped with "skip the rest". Put "Batch k of K" at the start of every question header.
   - **Order:** 8a core (five), 8b issues and concerns (five), then the 8c cards from `references/use-cases.md` whose Gate fires in this person's report. Skip cards with a zero gate; they are marked "not asked".
   - **Per card:** read the card's Picture line and its "In chat today" contrast from `references/use-cases.md` into the question text, so the person can imagine it. Fit has four options: fits, sometimes, no, hard to imagine without seeing it. Then one free-text question per card rated fits or sometimes, batched four per call, covering what it must show and what you would click. Skip follow-ups for no and hard to imagine.
   - **Options:** seed them from the person's own numbers (their top ask categories, their MCP servers, their counts) so most answers are one click; free text is always available. AskUserQuestion allows four options per question. When the evidence lists more than four candidates, split the question into part 1 and part 2 with the same stem and four options each.
   - Target under 20 minutes. Record the chosen option plus any free text verbatim. Never infer an answer; an unanswered question stays empty.
5. **Fill the template** into `./conversation-use-cases/conversation-use-cases-<YYYY-MM-DD>.md`. Every REQUIRED slot gets a value or `not measured` with one line saying why. Take counts from `session_miner_output.json` when a report table is cut (tool usage shows the top 25; the "Tracking and hand-off tools" table lists the tracking tools in full). Copy no project labels, session ids or paths from the report or the JSON; the template carries counts, categories and your one-line intents only. Section 7 takes the cards from `references/use-cases.md` and orders them by the rule in the template: Fit first (fits, sometimes, hard to imagine, no, then not asked), and within the same Fit by evidence share, the card's evidence count divided by the denominator named in `use-cases.md` (human turns, sessions or main-thread tool calls). Counts on different scales are never compared directly.
...
| Asking a card as a label only | Read its Picture and "In chat today" lines; offer "hard to imagine" as an answer and record it. |
