...
1. **Search before you build.** Before writing logic that handles a generic problem, check if a sub-workflow already exists. Use `search_workflows({ query: 'Subworkflow' })`, `query: '<keyword>'`, or `search_workflows({ tags: ['<tag>'] })` if the instance uses tags for categorization. The MCP now supports tag-based filtering via `search_workflows`, and you can enumerate available tags with `list_tags`.
...
search_workflows({ tags: ['<relevant-tag>'] })    // use list_tags to discover available tags
```

If something matches, use it. If not, build it as a sub-workflow so the *next* search finds it. The prefix convention (`Subworkflow:`, `Customer:`, etc.) is what makes that work. Tags are an additional discovery mechanism: use `list_tags` to see what tags exist on the instance, then `search_workflows({ tags: ['...'] })` to filter by them.
...
1. search_workflows with relevant queries (e.g. 'Subworkflow', the domain prefix, the operation keyword)
   Also try search_workflows({ tags: ['<tag>'] }) if the instance uses tags — use list_tags first to see what's available
...
| Building a new sub-workflow without searching | Library grows duplicates, and future searches find both | Always `search_workflows` first (by query and by tags via `list_tags` + `search_workflows({ tags: [...] })`) |
