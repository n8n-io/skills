...
1. **Search before you build.** Before writing logic that handles a generic problem, check if a sub-workflow already exists. Use `search_workflows({ query: 'Subworkflow' })`, `query: '<keyword>'`, or `search_workflows({ tags: ['<tag>'] })` if the user tags their workflows. The MCP now supports tag-based filtering, so tags are a valid discovery mechanism alongside name/description search.
...
search_workflows({ tags: ['utilities'] })    // if the user tags their workflows
```

If something matches, use it. If not, build it as a sub-workflow so the *next* search finds it. The prefix convention (`Subworkflow:`, `Customer:`, etc.) is what makes that work. Tags are now also a valid discovery mechanism via `search_workflows({ tags: [...] })`.
