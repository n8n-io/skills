...
search_workflows({ tags: ['<relevant-tag>'] })   // if the instance uses tags for categorisation
```

If something matches, use it. If not, build it as a sub-workflow so the *next* search finds it. The prefix convention (`Subworkflow:`, `Customer:`, etc.) is what makes that work. Tags are a useful complement when the instance uses them consistently.
...
1. search_workflows with relevant queries (e.g. 'Subworkflow', the domain prefix, the operation keyword)
   or search_workflows({ tags: ['<tag>'] }) if the instance uses tags for categorisation
...
| Building a new sub-workflow without searching | Library grows duplicates, and future searches find both | Always `search_workflows` first (by name prefix and/or tags) |
