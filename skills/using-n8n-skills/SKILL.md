...
| Tool | What it does |
|---|---|
| `search_workflows` | Search workflows across the instance by `query` (matches name and description) and/or `tags` (array of tag name strings; deduplicated, empty strings dropped). Results include a `tags` field (`[{ id, name }]`) on each workflow. The **only** cross-workflow tool. Use it to discover what already exists. |
