...
- **n8n-hosted MCP**: a workflow on the same n8n instance published with MCP access enabled (per `n8n-extending-mcp`). Same client node, just pointed at an n8n MCP trigger URL.
...
- For passing binary into tools: `n8n-binary-and-data` `references/AGENT_TOOL_BINARY.md`.
- For wrapping APIs as MCP tools accessible to agents: `n8n-extending-mcp`.
- For tool naming alignment with the rest of the workflow conventions: `n8n-workflow-lifecycle` `references/NAMING_CONVENTIONS.md`.
