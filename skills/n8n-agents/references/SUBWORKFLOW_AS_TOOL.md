...
For the broader error story (4xx/5xx mapping, retries, error workflows, deciding throw vs. continue), see the `n8n-error-handling` skill.
...
- For sub-workflow patterns generally (stateless, naming, search-before-build): `n8n-subworkflows`.
- For the Execute Workflow Trigger config details: `n8n-node-configuration` `references/TRIGGER_NODES.md`.
- For the Code-node-tool exception and the rules Code follows wherever it's used: `n8n-code-nodes`.
