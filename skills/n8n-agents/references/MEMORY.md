...
- To actually USE the file in a later tool call, it must still be in storage (per `n8n-binary-and-data` `AGENT_TOOL_BINARY.md`) and the storage key must be in THAT turn's system prompt.
...
- For session-keyed file storage: `n8n-binary-and-data` `references/AGENT_TOOL_BINARY.md`.
