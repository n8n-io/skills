...
- **Database or Data Table for exact lookups.** "Look up customer X's record", "fetch issue #1234", "get rows where status = 'open'" are not RAG problems. Use a database query or `n8n-data-tables` directly.
...
- For Data Tables (sometimes a viable alternative to a vector store for small structured data): `n8n-data-tables` SKILL.