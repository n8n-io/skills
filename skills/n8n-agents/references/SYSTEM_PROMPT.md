...
6. **File handling injection.** When chat includes uploaded files, inject storage keys so the agent can reference them in tool calls. See `n8n-binary-and-data` `AGENT_TOOL_BINARY.md`.
...
See `n8n-binary-and-data` `AGENT_TOOL_BINARY.md` for how to handle binary in agents
