...
- **Tags:** The MCP can read tags (via `list_tags`) and filter workflows by tag (via `search_workflows({ tags: [...] })`). Tags are still primarily a human-browsing tool in the UI, but they can now supplement name/description-based discovery for the agent. Don't rely on tags *alone* for AI discovery — name and description remain the primary search surface.
