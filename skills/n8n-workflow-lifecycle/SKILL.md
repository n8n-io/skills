...
- **Tags:** browsable in the UI and now also filterable via the MCP (`search_workflows` accepts a `tags` filter, and `list_tags` enumerates all tags with usage counts). Useful for both human browsing and agent-driven discovery.
