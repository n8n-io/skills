...
- **Tags:** Useful for both UI browsing and MCP-based discovery. Use `list_tags` to enumerate existing tags, and `search_workflows({ tags: ['tag-name'] })` to filter workflows by tag. Tags are also visible to humans browsing the UI.
