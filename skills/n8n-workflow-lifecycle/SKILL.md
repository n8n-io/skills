...
- **Tags:** browsable in the UI and now also filterable via the MCP (`search_workflows({ tags: [...] })` and `list_tags`). Useful for both humans browsing the UI and agents doing tag-based discovery. Don't rely on them as the *sole* discovery mechanism for agents — keyword search via `query` is still the primary path — but they are a valid complement.
