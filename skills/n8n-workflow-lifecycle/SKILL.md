...
- **Tags:** readable in the UI and now also filterable via `search_workflows({ tags: ['<name>'] })` and listable via `list_tags`. Useful for both humans browsing the UI and agents filtering workflows by tag. Don't rely on tags as the *sole* discovery mechanism for agents (tag names must be known in advance), but they are a useful complement to name/description search.
